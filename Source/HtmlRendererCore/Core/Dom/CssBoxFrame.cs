...
namespace HtmlRendererCore.Core.Dom
{
    ...
    internal sealed class CssBoxFrame : CssBox
    {
        ...
        {
            get { return this._videoLinkUrl ?? this.GetAttribute("src"); }
        }
        ...
        private void HandleDataLoadFailure(Exception ex, string source)
        {
            ...
            {
                this._videoTitle = "The video is not found, possibly removed by the user.";
            }
            else
            {
        ...
        {
            if (this._videoImageUrl != null && this._imageLoadHandler == null)
            {
                this._imageLoadHandler = new ImageLoadHandler(this.HtmlContainer, this.OnLoadImageComplete);
                this._imageLoadHandler.LoadImage(this._videoImageUrl, this.HtmlTag != null ? this.HtmlTag.Attributes : null);
            }
        ...
        private void DrawImage(RGraphics g, RPoint offset, RRect rect)
        {
            if (this._imageWord.Image != null)
            {
                if (rect.Width > 0 && rect.Height > 0)
                {
                    if (this._imageWord.ImageRectangle == RRect.Empty)
                        g.DrawImage(this._imageWord.Image, rect);
                    else
                        g.DrawImage(this._imageWord.Image, rect, this._imageWord.ImageRectangle);
            ...
            else if (this._isVideo && !this._imageLoadingComplete)
            {
        ...
        {
            if (this._videoTitle != null && this._imageWord.Width > 40 && this._imageWord.Height > 40)
            {
                ...
                g.DrawString(this._videoTitle, font, RColor.WhiteSmoke, titleRect.Location, RSize.Empty, false);
            }
        }
        ...
        {
            if (this._isVideo && this._imageWord.Width > 70 && this._imageWord.Height > 50)
            {
