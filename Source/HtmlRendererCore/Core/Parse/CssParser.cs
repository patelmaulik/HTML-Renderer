...
namespace HtmlRendererCore.Core.Parse
{
    ...
    internal sealed class CssParser
    {
        ...
        public CssParser(RAdapter adapter)
        {
            ...
            this._valueParser = new CssValueParser(adapter);
        ...
        {
            var cssData = combineWithDefault ? this._adapter.DefaultCssData.Clone() : new CssData();
        ...
        {
            return this._valueParser.GetActualColor(colorStr);
        }
        ...
        {
            if (this._valueParser.IsColorValid(propValue))
            {
                properties[propName] = propValue;
            }
        }
        ...
        private string ParseFontFamilyProperty(string propValue)
        {
            ...
            while (start > -1 && start < propValue.Length)
            {
                ...
                if (this._adapter.IsFontExists(font))
                {
                    return font;
                }
        ...
        {
            RColor color;
            return this._valueParser.TryGetColor(str, idx, length, out color) ? str.Substring(idx, length) : null;
        }
