...
namespace HtmlRendererCore.Core.Handlers
{
    ...
    internal sealed class ImageDownloader : IDisposable
    {
        ...
        public void DownloadImage(Uri imageUri, string filePath, bool async, DownloadFileAsyncCallback cachedFileCallback)
        {
            ...
            bool download = true;
            lock (this._imageDownloadCallbacks)
            {
                if (this._imageDownloadCallbacks.ContainsKey(filePath))
                {
                    download = false;
                    this._imageDownloadCallbacks[filePath].Add(cachedFileCallback);
                }
                else
                {
                    this._imageDownloadCallbacks[filePath] = new List<DownloadFileAsyncCallback> { cachedFileCallback };
                }
            }
        ...
        private void DownloadImageFromUrlAsync(object data)
        {
            ...
            {
                this.OnDownloadImageCompleted(null, downloadData._uri, downloadData._tempPath, downloadData._filePath, ex, false);
            }
        }
        ...
        private void OnDownloadImageAsyncCompleted(object sender, AsyncCompletedEventArgs e)
        {
            ...
            try
            {
                ...
                {
                    client.DownloadFileCompleted -= this.OnDownloadImageAsyncCompleted;
                    this.OnDownloadImageCompleted(client, downloadData._uri, downloadData._tempPath, downloadData._filePath, e.Error, e.Cancelled);
                }
            }
            catch (Exception ex)
            {
        ...
        private void OnDownloadImageCompleted(WebClient client, Uri source, string tempPath, string filePath, Exception error, bool cancelled)
        {
            ...
            lock (this._imageDownloadCallbacks)
            {
                if (this._imageDownloadCallbacks.TryGetValue(filePath, out callbacksList))
                    this._imageDownloadCallbacks.Remove(filePath);
            }
        ...
        {
            this._imageDownloadCallbacks.Clear();
            while (this._clients.Count > 0)
            {
                try
                {
                    var client = this._clients[0];
                    client.CancelAsync();
                    client.Dispose();
