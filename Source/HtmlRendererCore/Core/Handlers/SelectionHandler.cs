...
namespace HtmlRendererCore.Core.Handlers
{
    ...
    internal sealed class SelectionHandler : IDisposable
    {
        ...
        public SelectionHandler(CssBox root)
        {
            ...
            this._root = root;
            this._contextMenuHandler = new ContextMenuHandler(this, root.HtmlContainer);
        }
        ...
        {
            if (this._root.HtmlContainer.IsSelectionEnabled)
            {
                this.ClearSelection();
                this.SelectAllWords(this._root);
                control.Invalidate();
            }
        }
        ...
        public void SelectWord(RControl control, RPoint loc)
        {
            if (this._root.HtmlContainer.IsSelectionEnabled)
            {
                var word = DomUtils.GetCssBoxWord(this._root, loc);
                if (word != null)
                {
                    word.Selection = this;
                    this._selectionStartPoint = loc;
                    this._selectionStart = this._selectionEnd = word;
                    control.Invalidate();
                }
            }
        }
        ...
        public void HandleMouseDown(RControl parent, RPoint loc, bool isMouseInContainer)
        {
            ...
            {
                this._mouseDownInControl = true;
                this._isDoubleClickSelect = (DateTime.Now - this._lastMouseDown).TotalMilliseconds < 400;
                this._lastMouseDown = DateTime.Now;
                this._mouseDownOnSelectedWord = false;

                if (this._root.HtmlContainer.IsSelectionEnabled && parent.LeftMouseButton)
                {
                    var word = DomUtils.GetCssBoxWord(this._root, loc);
                    if (word != null && word.Selected)
                    {
                        this._mouseDownOnSelectedWord = true;
                    }
                    else
                    {
                        clear = true;
                ...
                {
                    var rect = DomUtils.GetCssBoxWord(this._root, loc);
                    var link = DomUtils.GetLinkBox(this._root, loc);
                    if (this._root.HtmlContainer.IsContextMenuEnabled)
                    {
                        this._contextMenuHandler.ShowContextMenu(parent, rect, link);
                    }
        ...
        {
            bool ignore = false;
            this._mouseDownInControl = false;
            if (this._root.HtmlContainer.IsSelectionEnabled)
            {
                ignore = this._inSelection;
                if (!this._inSelection && leftMouseButton && this._mouseDownOnSelectedWord)
                {
                    this.ClearSelection();
                    parent.Invalidate();
                }
            ...
            ignore = ignore || (DateTime.Now - this._lastMouseDown > TimeSpan.FromSeconds(1));
            return ignore;
        }
        ...
        {
            if (this._root.HtmlContainer.IsSelectionEnabled && this._mouseDownInControl && parent.LeftMouseButton)
            {
                if (this._mouseDownOnSelectedWord)
                {
                    // make sure not to start drag-drop on click but when it actually moves as it fucks mouse-up
                    if ((DateTime.Now - this._lastMouseDown).TotalMilliseconds > 200)
                        this.StartDragDrop(parent);
                }
                else
                {
                    this.HandleSelection(parent, loc, !this._isDoubleClickSelect);
                    this._inSelection = this._selectionStart != null && this._selectionEnd != null && (this._selectionStart != this._selectionEnd || this._selectionStartIndex != this._selectionEndIndex);
                }
            }
            else
            {
                ...
                else if (this._root.HtmlContainer.IsSelectionEnabled)
                {
                    var word = DomUtils.GetCssBoxWord(this._root, loc);
                    this._cursorChanged = word != null && !word.IsImage && !(word.Selected && (word.SelectedStartIndex < 0 || word.Left + word.SelectedStartOffset <= loc.X) && (word.SelectedEndOffset < 0 || word.Left + word.SelectedEndOffset >= loc.X));
        ...
        public void CopySelectedHtml()
        {
            if (this._root.HtmlContainer.IsSelectionEnabled)
            {
                var html = DomUtils.GenerateHtml(this._root, HtmlGenerationStyle.Inline, true);
                var plainText = DomUtils.GetSelectedPlainText(this._root);
                if (!string.IsNullOrEmpty(plainText))
                    this._root.HtmlContainer.Adapter.SetToClipboard(html, plainText);
            }
        }
        ...
        {
            return this._root.HtmlContainer.IsSelectionEnabled ? DomUtils.GetSelectedPlainText(this._root) : null;
        }
        ...
        {
            return this._root.HtmlContainer.IsSelectionEnabled ? DomUtils.GenerateHtml(this._root, HtmlGenerationStyle.Inline, true) : null;
        }
        ...
        {
            return word == (this._backwardSelection ? this._selectionEnd : this._selectionStart) ? (this._backwardSelection ? this._selectionEndIndex : this._selectionStartIndex) : -1;
        }
        ...
        {
            return word == (this._backwardSelection ? this._selectionStart : this._selectionEnd) ? (this._backwardSelection ? this._selectionStartIndex : this._selectionEndIndex) : -1;
        }
        ...
        {
            return word == (this._backwardSelection ? this._selectionEnd : this._selectionStart) ? (this._backwardSelection ? this._selectionEndOffset : this._selectionStartOffset) : -1;
        }
        ...
        {
            return word == (this._backwardSelection ? this._selectionStart : this._selectionEnd) ? (this._backwardSelection ? this._selectionStartOffset : this._selectionEndOffset) : -1;
        }
        ...
        public void ClearSelection()
        {
            ...
            ClearSelection(this._root);

            this._selectionStartOffset = -1;
            this._selectionStartIndex = -1;
            this._selectionEndOffset = -1;
            this._selectionEndIndex = -1;

            this._selectionStartPoint = RPoint.Empty;
        ...
        public void Dispose()
        {
            this._contextMenuHandler.Dispose();
        }
        ...
        private void HandleSelection(RControl control, RPoint loc, bool allowPartialSelect)
        {
            ...
            var lineBox = DomUtils.GetCssLineBox(this._root, loc);
            if (lineBox != null)
            {
                ...
                if (word != null)
                {
                    ...
                    if (this.CheckNonEmptySelection(loc, allowPartialSelect))
                    {
                        ...
                        this.SelectWordsInRange(this._root, this._backwardSelection ? this._selectionEnd : this._selectionStart, this._backwardSelection ? this._selectionStart : this._selectionEnd);
                    }
                    else
                    {
                        this._selectionEnd = null;
        ...
        {
            if (this._dragDropData == null)
            {
                var html = DomUtils.GenerateHtml(this._root, HtmlGenerationStyle.Inline, true);
                var plainText = DomUtils.GetSelectedPlainText(this._root);
                this._dragDropData = control.Adapter.GetClipboardDataObject(html, plainText);
            }
            control.DoDragDropCopy(this._dragDropData);
        }
        ...
        private bool CheckNonEmptySelection(RPoint loc, bool allowPartialSelect)
        {
            ...
            if (Math.Abs(this._selectionStartPoint.X - loc.X) <= 1 && Math.Abs(this._selectionStartPoint.Y - loc.Y) < 5)
                return false;

            // selection is empty if on same word and same index
            return this._selectionStart != this._selectionEnd || this._selectionStartIndex != this._selectionEndIndex;
        }
        ...
        /// <seealso cref="CalculateWordCharIndexAndOffset(HtmlRendererCore.Adapters.RControl,HtmlRendererCore.Core.Dom.CssRect,HtmlRendererCore.Adapters.Entities.RPoint,bool)"/>.
        /// </summary>
        ...
        private void CalculateWordCharIndexAndOffset(RControl control, CssRect word, RPoint loc, bool selectionStart)
        {
            ...
            if (selectionStart)
            {
                this._selectionStartIndex = selectionIndex;
                this._selectionStartOffset = selectionOffset;
            }
            else
            {
                this._selectionEndIndex = selectionIndex;
                this._selectionEndOffset = selectionOffset;
            }
        }
        ...
        private void CheckSelectionDirection()
        {
            ...
            {
                this._backwardSelection = this._selectionStartIndex > this._selectionEndIndex;
            }
            else if (DomUtils.GetCssLineBoxByWord(this._selectionStart) == DomUtils.GetCssLineBoxByWord(this._selectionEnd))
            {
                this._backwardSelection = this._selectionStart.Left > this._selectionEnd.Left;
            }
            else
            {
                this._backwardSelection = this._selectionStart.Top >= this._selectionEnd.Bottom;
            }
        }
