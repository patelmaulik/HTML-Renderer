    ...
    internal sealed class ImageLoadHandler : IDisposable
    {
        ...
        public void LoadImage(string src, Dictionary<string, string> attributes)
        {
            try
            {
                ...
                this._htmlContainer.RaiseHtmlImageLoadEvent(args);
                this._asyncCallback = !this._htmlContainer.AvoidAsyncImagesLoading;
            ...
            {
                this._htmlContainer.ReportError(HtmlRenderErrorType.Image, "Exception in handling image source", ex);
                this.ImageLoadComplete(false);
            }
        }
        ...
        private void OnHtmlImageLoadEventCallback(string path, object image, RRect imageRectangle)
        {
            if (!this._disposed)
            {
                this._imageRectangle = imageRectangle;

                if (image != null)
                {
                    this._image = this._htmlContainer.Adapter.ConvertImage(image);
                    this.ImageLoadComplete(this._asyncCallback);
                }
        ...
        private void SetFromInlineData(string src)
        {
            ...
            if (this._image == null)
                this._htmlContainer.ReportError(HtmlRenderErrorType.Image, "Failed extract image from inline data");
            this._releaseImageObject = true;
            this.ImageLoadComplete(false);
        }
        ...
        private RImage GetImageFromData(string src)
        {
            ...
            if (s.Length == 2)
            {
                ...
                if (imagePartsCount > 0)
                {
                    ...
                    return this._htmlContainer.Adapter.ImageFromStream(new MemoryStream(imageData));
                }
            }
            return null;
        }
        ...
        private void SetImageFromPath(string path)
        {
            ...
            else
            {
                ...
                {
                    this._htmlContainer.ReportError(HtmlRenderErrorType.Image, "Failed load image, invalid source: " + path);
                    this.ImageLoadComplete(false);
                }
            }
        }
        ...
        private void SetImageFromFile(FileInfo source)
        {
            if (source.Exists)
            {
                if (this._htmlContainer.AvoidAsyncImagesLoading)
        ...
        private void LoadImageFromFile(string source)
        {
            try
            {
                ...
                lock (this._loadCompleteCallback)
                {
                    this._imageFileStream = imageFileStream;
                    if (!this._disposed)
                        this._image = this._htmlContainer.Adapter.ImageFromStream(this._imageFileStream);
                    this._releaseImageObject = true;
                }
                this.ImageLoadComplete();
            }
            ...
            {
                this._htmlContainer.ReportError(HtmlRenderErrorType.Image, "Failed to load image from disk: " + source, ex);
                this.ImageLoadComplete();
            }
        }
        ...
        private void SetImageFromUrl(Uri source)
        {
            ...
            {
                this._htmlContainer.GetImageDownloader().DownloadImage(source, filePath.FullName, !this._htmlContainer.AvoidAsyncImagesLoading, this.OnDownloadImageCompleted);
            }
        }
        ...
        private void OnDownloadImageCompleted(Uri imageUri, string filePath, Exception error, bool canceled)
        {
            if (!canceled && !this._disposed)
            {
                ...
                {
                    this._htmlContainer.ReportError(HtmlRenderErrorType.Image, "Failed to load image from URL: " + imageUri, error);
                    this.ImageLoadComplete();
                }
            }
        }
        ...
        private void ImageLoadComplete(bool async = true)
        {
            ...
            if (this._disposed)
                ...
                this._loadCompleteCallback(this._image, this._imageRectangle, async);
        }
        ...
        private void ReleaseObjects()
        {
            lock (this._loadCompleteCallback)
            {
                if (this._releaseImageObject && this._image != null)
                {
                    this._image.Dispose();
                    this._image = null;
                }
                if (this._imageFileStream != null)
                {
                    this._imageFileStream.Dispose();
                    this._imageFileStream = null;
                }
            }
        }
