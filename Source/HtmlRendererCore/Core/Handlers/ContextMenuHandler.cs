    ...
    internal sealed class ContextMenuHandler : IDisposable
    {
        ...
        public void ShowContextMenu(RControl parent, CssRect rect, CssBox link)
        {
            try
            {
                ...
                this._contextMenu = this._htmlContainer.Adapter.GetContextMenu();
                ...
                if (rect != null)
                {
                    ...
                    if (link != null)
                    {
                        ...
                        this._contextMenu.AddItem(isVideo ? _openVideo : _openLink, linkExist, this.OnOpenLinkClick);
                        if (this._htmlContainer.IsSelectionEnabled)
                        {
                            this._contextMenu.AddItem(isVideo ? _copyVideoUrl : _copyLink, linkExist, this.OnCopyLinkClick);
                        }
                        this._contextMenu.AddDivider();
                    }

                    if (rect.IsImage && !isVideo)
                    {
                        this._contextMenu.AddItem(_saveImage, rect.Image != null, this.OnSaveImageClick);
                        if (this._htmlContainer.IsSelectionEnabled)
                        {
                            this._contextMenu.AddItem(_copyImageLink, !string.IsNullOrEmpty(this._currentRect.OwnerBox.GetAttribute("src")), this.OnCopyImageLinkClick);
                            this._contextMenu.AddItem(_copyImage, rect.Image != null, this.OnCopyImageClick);
                        }
                        this._contextMenu.AddDivider();
                    }
                    ...
                    if (this._htmlContainer.IsSelectionEnabled)
                    {
                        this._contextMenu.AddItem(_copy, rect.Selected, this.OnCopyClick);
                    }
                }

                if (this._htmlContainer.IsSelectionEnabled)
                {
                    this._contextMenu.AddItem(_selectAll, true, this.OnSelectAllClick);
                }

                if (this._contextMenu.ItemsCount > 0)
                {
                    this._contextMenu.RemoveLastDivider();
                    this._contextMenu.Show(parent, parent.MouseLocation);
                }
            }
            catch (Exception ex)
            {
                this._htmlContainer.ReportError(HtmlRenderErrorType.ContextMenu, "Failed to show context menu", ex);
            }
        }
        ...
        private void OnOpenLinkClick(object sender, EventArgs eventArgs)
        {
            try
            {
                this._currentLink.HtmlContainer.HandleLinkClicked(this._parentControl, this._parentControl.MouseLocation, this._currentLink);
            }
            ...
            {
                this._htmlContainer.ReportError(HtmlRenderErrorType.ContextMenu, "Failed to open link", ex);
            }
            finally
            {
                this.DisposeContextMenu();
        ...
        private void OnCopyLinkClick(object sender, EventArgs eventArgs)
        {
            try
            {
                this._htmlContainer.Adapter.SetToClipboard(this._currentLink.HrefLink);
            }
            catch (Exception ex)
            {
                this._htmlContainer.ReportError(HtmlRenderErrorType.ContextMenu, "Failed to copy link url to clipboard", ex);
            }
            finally
            {
                this.DisposeContextMenu();
        ...
        private void OnSaveImageClick(object sender, EventArgs eventArgs)
        {
            try
            {
                var imageSrc = this._currentRect.OwnerBox.GetAttribute("src");
                this._htmlContainer.Adapter.SaveToFile(this._currentRect.Image, Path.GetFileName(imageSrc) ?? "image", Path.GetExtension(imageSrc) ?? "png");
            }
            catch (Exception ex)
            {
                this._htmlContainer.ReportError(HtmlRenderErrorType.ContextMenu, "Failed to save image", ex);
            }
            finally
            {
                this.DisposeContextMenu();
        ...
        private void OnCopyImageLinkClick(object sender, EventArgs eventArgs)
        {
            try
            {
                this._htmlContainer.Adapter.SetToClipboard(this._currentRect.OwnerBox.GetAttribute("src"));
            }
            catch (Exception ex)
            {
                this._htmlContainer.ReportError(HtmlRenderErrorType.ContextMenu, "Failed to copy image url to clipboard", ex);
            }
            finally
            {
                this.DisposeContextMenu();
        ...
        private void OnCopyImageClick(object sender, EventArgs eventArgs)
        {
            try
            {
                this._htmlContainer.Adapter.SetToClipboard(this._currentRect.Image);
            }
            catch (Exception ex)
            {
                this._htmlContainer.ReportError(HtmlRenderErrorType.ContextMenu, "Failed to copy image to clipboard", ex);
            }
            finally
            {
                this.DisposeContextMenu();
        ...
        private void OnCopyClick(object sender, EventArgs eventArgs)
        {
            ...
            {
                this._htmlContainer.ReportError(HtmlRenderErrorType.ContextMenu, "Failed to copy text to clipboard", ex);
            }
            finally
            {
                this.DisposeContextMenu();
        ...
        private void OnSelectAllClick(object sender, EventArgs eventArgs)
        {
            try
            {
                this._selectionHandler.SelectAll(this._parentControl);
            }
            catch (Exception ex)
            {
                this._htmlContainer.ReportError(HtmlRenderErrorType.ContextMenu, "Failed to select all text", ex);
            }
            finally
            {
                this.DisposeContextMenu();
