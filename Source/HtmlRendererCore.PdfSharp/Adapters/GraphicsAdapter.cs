...
namespace HtmlRendererCore.PdfSharp.Adapters
{
    ...
    internal sealed class GraphicsAdapter : RGraphics
    {
        ...
        public override void PushClip(RRect rect)
        {
            ...
            this._g.Save();
            this._g.IntersectClip(Utils.Convert(rect));
        }
        ...
        {
            var prevMode = this._g.SmoothingMode;
            this._g.SmoothingMode = XSmoothingMode.AntiAlias;
            return prevMode;
        }
        ...
        {
            this._g.DrawLine(((PenAdapter)pen).Pen, x1, y1, x2, y2);
        }

        public override void DrawRectangle(RPen pen, double x, double y, double width, double height)
        {
            this._g.DrawRectangle(((PenAdapter)pen).Pen, x, y, width, height);
        }
        ...
        {
            this._g.DrawImage(((ImageAdapter)image).Image, Utils.Convert(destRect), Utils.Convert(srcRect), XGraphicsUnit.Point);
        }

        public override void DrawImage(RImage image, RRect destRect)
        {
            this._g.DrawImage(((ImageAdapter)image).Image, Utils.Convert(destRect));
        }

        public override void DrawPath(RPen pen, RGraphicsPath path)
        {
            this._g.DrawPath(((PenAdapter)pen).Pen, ((GraphicsPathAdapter)path).GraphicsPath);
        }
